...
换句话说，以一种悲观的态度来看，本应该采用的多种优化手段都因为`eval(..)`或`with`的存在而无法使用，因此它几乎不会采用任何的优化手段。

你的代码几乎一定会因为你使用了`eval(..)`或`with`而变得更慢。无论引擎会有多智能的尝试消除这些悲观假设的副作用，都无法改变这一事实：由于没有优化，代码运行得更加缓慢。

# 章节回顾（给太长不看的人看的）

词法作用域的意思是：作用域是在编写代码的时刻被函数声明的位置所定义的。在编译的词法分析阶段本质上是知晓所有这些标识符是如何被定义的，从而预测它们该如何在执行阶段被找到。

JavaScript有两种机制可以“欺骗”词法作用域：`eval(..)`和`with`。前者可以通过生成一段包含一个或多个声明的字符串“代码”（在运行时）修改已经存在的词法作用域。后者本质上是通过将一个对象引用视为一个“作用域”而对象的属性作为标识符的方式（同样，在运行时）创建了一个全新的词法作用域。

这些机制的缺点在于它们使得引擎的编译器优化（通过作用域查找的方面）机制失效，因为引擎悲观地假设这样的优化是无效的。代码将因为你使用了其中任何一个特性而运行缓慢。因此，**不要使用他们。**